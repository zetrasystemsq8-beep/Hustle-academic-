...
SOURCE_FILE = Path("marketing-skills/Copywriting.md")
...
def escape_dart_string(text: str) -> str:
    ...
    Order matters:
      ...
      3. Escape '$' so Dart never attempts string interpolation — this
         alone makes triple-quote-looking sequences, PHP `$var`, and
         shell-style variables completely inert.
      4. Escape double quotes (our string delimiter). Each quote is
         escaped individually, so any run of quotes (e.g. a stray
         `"""`) is handled safely no matter how many appear in a row.
      ...
      6. Strip ONLY genuine control characters (Unicode category "Cc")
         ...
         does NOT touch zero-width joiners (U+200D), variation
         selectors (U+FE0F), combining marks, or any other printable
         Unicode/emoji codepoint, so multi-codepoint emoji sequences
         (e.g. family emoji, flags, skin-tone modifiers) stay intact.
    ...
    #    including triple-quote-like sequences, one quote at a time).
    escaped = escaped.replace('"', '\\"')
